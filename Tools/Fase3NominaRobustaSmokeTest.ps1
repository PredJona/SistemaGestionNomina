...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Proy2_Fase3_" + [Guid]::NewGuid().ToString("N"))
$dbPath = Join-Path $tempRoot "nomina_fase3.db"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "No se pudo compilar el host real de Fase 3." }
    ...
    & $hostExe $dbPath
    if ($LASTEXITCODE -ne 0) { throw "El smoke test real de Fase 3 devolvió código $LASTEXITCODE." }
    Write-Host "Fase 3 Nomina Robusta Smoke Test OK" -ForegroundColor Green
}
finally {
